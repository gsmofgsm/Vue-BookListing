...
describe("BookList.vue", () => {
  it("should contain BookItem component @book-list-vue-contains-book-item-export", () => {
    ...
    const results = esquery(
      ast,
      "Property[key.name=components] > ObjectExpression > Property[key.name=BookItem]"
    );
    assert(
      results.length > 0,
      "The value of the `components` property is not an object containing `BookItem` in BookList.vue"
    );
  });
});
