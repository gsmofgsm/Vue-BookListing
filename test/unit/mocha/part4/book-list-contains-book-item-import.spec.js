...
describe("BookList.vue", () => {
  it("should contain BookItem component @book-list-vue-contains-book-item-import", () => {
    ...
    let ast;
    try {
      ast = esprima.parse(script[0].childNodes[0].value, {
        sourceType: "module",
      });
    } catch (e) {
      assert(
        false,
        "It looks like your import statement is missing a semicolon at the end."
      );
    }
    ...
    assert(
      results.length > 0,
      "The `BookItem` class was not imported from `./BookItem` In BookList.vue"
    );

    results = esquery(ast, "ImportSpecifier");
    assert(
      results.length == 0,
      "Your import statement currently uses curly braces `{ }` to import the `BookItem` component - please replace that with just the name `BookItem`"
    );
  });
});
